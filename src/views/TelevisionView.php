...
class TelevisionView extends UserView
{
     ...
     * @return string
     */
    public function displayTableTv($users, $name="erreur du nom") {
        ...
        foreach ($users as $user) {
            ++$count;
            $row[] = [$count, $user->getLogin(), sizeof($user->getCodes()),"<a href='".add_query_arg('id', $user->getId(), home_url('/users/edit'))."'><button type='button' class='btn btn-primary'>Modifier</button></a>"];
        }

        return $this->displayTable('TV', $title, $header, $row, 'tele', '<a type="submit" class="btn btn-primary" role="button" aria-disabled="true" href="' . home_url('/creer-utilisateur') . '">Créer</a>');
    }
