...
class UserView extends View
{
    ...
    {
        $title = '<b>Liste des utilisateurs :</b>';
        $id = 'all';

        $header = ['Identifiant', 'Nom', 'Email', 'Rôle', 'Supprimer'];

        $row = array();

        foreach ($users as $user) {
            if (implode("'", $this->getRoles($user)) !== "administrator"){
                $deleteButton = "<button name='idToDelete' value='".$user->ID."' type='submit' class='btn btn-danger'>Supprimer</button>";
            }
            else{
                $deleteButton = "<button onclick='alert(\"Vous ne pouvez pas supprimer un administrateur du site !\")' class='btn btn-danger'>Supprimer</button>";
            }
            $row[] = [
                $user->ID,
                $user->get('user_login'),
        ...
        return $this->displayTable($id, $title, $header, $row, $id, '<a type="submit" class="btn btn-primary" role="button" aria-disabled="true" href="' . home_url('/creer-utilisateur') . '">Créer</a>');
    }
}
