...
class CodeAdeView extends View
{
    ...
    {
        $header = ['Titre', 'Code', 'Type', 'Modifier', 'Supprimer'];
        ...
        foreach ($groups as $codeAde) {
            foreach ($codeAde as $code) {
                ++$count;
            $row[] = [$count, $code->getTitle(), $code->getCode(), $code->getType(), "<a href='".home_url("/gestion-codes-ade/modification-code-ade")."?id=".$code->getId()."'><button class='btn btn-primary' type='button'>Modifier</button></a>", "<button name='idToDelete' value='".$code->getId()."' type='submit' class='btn btn-danger'>Supprimer</button>"];
            }
        }
